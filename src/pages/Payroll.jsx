...
import Loadertwo from "../components/Loadertwo"; // Make sure this is imported
import {
  Chart as ChartJS,
  ArcElement,
  Tooltip,
...
const Payroll = () => {
  ...
  useEffect(() => {
    ...
    const fetchPayrollData = async () => {
      const userDocRef = doc(db, "payroll", userEmail);
      const docSnap = await getDoc(userDocRef);

      if (docSnap.exists()) {
        const data = docSnap.data();
        setPayrollData(data.current);
        ...
        await setDoc(userDocRef, newPayrollData);

        setPayrollData(newPayrollData.current);
        setHistory(newPayrollData.history);
      }
